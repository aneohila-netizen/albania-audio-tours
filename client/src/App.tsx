...
// ─── Draggable WhatsApp button (mobile only) ───────────────────────────────────
const WA_SIZE = 48;

function DraggableWhatsApp() {
  // Start position: right edge, above bottom nav
  const [pos, setPos] = useState({ x: window.innerWidth - WA_SIZE - 12, y: window.innerHeight - 200 });
  const dragRef = useRef<{ startTouch: { x: number; y: number }; startPos: { x: number; y: number }; moved: boolean } | null>(null);

  const onTouchStart = (e: React.TouchEvent) => {
    const t = e.touches[0];
    dragRef.current = { startTouch: { x: t.clientX, y: t.clientY }, startPos: { ...pos }, moved: false };
  };

  const onTouchMove = (e: React.TouchEvent) => {
    if (!dragRef.current) return;
    const t = e.touches[0];
    const dx = t.clientX - dragRef.current.startTouch.x;
    const dy = t.clientY - dragRef.current.startTouch.y;
    if (Math.abs(dx) > 4 || Math.abs(dy) > 4) dragRef.current.moved = true;
    const nx = Math.max(0, Math.min(window.innerWidth  - WA_SIZE, dragRef.current.startPos.x + dx));
    const ny = Math.max(0, Math.min(window.innerHeight - WA_SIZE, dragRef.current.startPos.y + dy));
    setPos({ x: nx, y: ny });
  ...
  const onTouchEnd = (e: React.TouchEvent) => {
    ...
    if (!dragRef.current.moved) {
      // Tap — open WhatsApp
      window.open("https://wa.me/355686064077", "_blank", "noopener,noreferrer");
    } else {
      // Drag end — snap to nearest horizontal edge
      const mid = window.innerWidth / 2;
      const snappedX = pos.x + WA_SIZE / 2 < mid ? 12 : window.innerWidth - WA_SIZE - 12;
  ...
  return (
    <div
      ...
      style={{
        ...
        touchAction: "none",
        userSelect: "none",
        transition: "left 0.2s cubic-bezier(0.32,0.72,0,1)", // smooth edge-snap only
      }}
    >
      ...
      <AudioPlayerProvider onComplete={handleAudioComplete} onNavigate={handleNavigate}>
        ...
        <Switch>
          ...
          <Route>
            ...
            {/* WhatsApp floating button — mobile only, draggable with edge-snap */}
            <DraggableWhatsApp />
